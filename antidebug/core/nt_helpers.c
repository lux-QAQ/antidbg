...
// 定义 VCH 函数指针类型
typedef PVOID(NTAPI *pRtlAddVectoredContinueHandler)(
    ULONG First, PVECTORED_EXCEPTION_HANDLER Handler);
typedef ULONG(NTAPI *pRtlRemoveVectoredContinueHandler)(PVOID Handle);

// 实现 Dbg_AddVectoredContinueHandler
PVOID Dbg_AddVectoredContinueHandler(ULONG First,
                                     PVECTORED_EXCEPTION_HANDLER Handler) {
  pRtlAddVectoredContinueHandler fn =
      (pRtlAddVectoredContinueHandler)DbgGetNtdllExport(
          Dbg_HashSyscall("RtlAddVectoredContinueHandler"));
...
// [新增] 实现 Dbg_RemoveVectoredContinueHandler
ULONG Dbg_RemoveVectoredContinueHandler(PVOID Handle) {
  pRtlRemoveVectoredContinueHandler fn =
      (pRtlRemoveVectoredContinueHandler)DbgGetNtdllExport(
          Dbg_HashSyscall("RtlRemoveVectoredContinueHandler"));
...
void ForceExit() {
  ...
  DbgNtSetInformationProcess(hProcess, (PROCESSINFOCLASS)7, &debugPort,
                             sizeof(debugPort));
